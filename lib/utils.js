...
export function optimizeImageUrl(url, { width = 1920, quality = 'auto', format = 'auto' } = {}) {
  if (!url || typeof url !== 'string') return url;
  if (url.includes('res.cloudinary.com') && url.includes('/image/upload/')) {
    const transformStr = `f_${format},q_${quality},w_${width},c_limit`;
    // If it already has an existing transformation segment right after /image/upload/, replace it
    if (/\/image\/upload\/[a-z0-9_,:]+\//.test(url)) {
      return url.replace(/\/image\/upload\/[a-z0-9_,:]+\//, `/image/upload/${transformStr}/`);
    }
