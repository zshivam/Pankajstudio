 ...
 * GalleryUploader
 * Props:
 *   images         — current array of image objects or strings
...
export default function GalleryUploader({ images = [], onChange, category = 'general' }) {
  ...
  const uploadFile = useCallback(async (file) => {
    ...
    // 🌟 SAFE RETURN: Handles both { image: { url } } AND { url: "..." }
    if (typeof data.image === 'object' && data.image !== null) {
      return data.image;
    } else if (data.url) {
      return { url: data.url, altText: '' };
    } else if (typeof data.image === 'string') {
      return { url: data.image, altText: '' };
    }
  ...
  async function handleFiles(files) {
    ...
    // 🌟 FIX: Reset input value so same file can be re-selected if deleted
    if (inputRef.current) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button
          ...
          style={{
            ...
            cursor: uploading || images.length >= 60 ? 'not-allowed' : 'pointer',
            transition: 'all 0.2s',
            opacity: uploading || images.length >= 60 ? 0.5 : 1
          }}
        >
      ...
      {errors.length > 0 && (
        <div style={{ background: 'rgba(255,100,100,0.1)', border: '1px solid rgba(255,100,100,0.2)', padding: '12px 16px', borderRadius: 4 }}>
          {errors.map((e, i) => (
            <p key={i} style={{ fontFamily: '"DM Mono", monospace', fontSize: 10, color: '#ff8080', margin: '0 0 4px 0' }}>{e}</p>
          ))}
        </div>
      )}
      ...
      {images.length > 0 ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 10 }}>
          {images.map((img, i) => {
            // 🌟 SAFE RESOLUTION: Extract image URL whether img is string or object
