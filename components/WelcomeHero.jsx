...
export default function WelcomeHero({ images = [] }) {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-neutral-950 flex flex-col items-center justify-center">
      ...
      {images.length > 0 && images.map((img, index) => (
        <div key={img._id || index} className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${index === currentIndex ? "opacity-100" : "opacity-0"}`}>
          <img src={img.imageUrl || img} alt="Pankaj Studio Carousel" className="w-full h-full object-cover opacity-40" />
        </div>
      ))}
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-4">
       ...
       <div className="mb-2 relative w-36 h-36 md:w-48 md:h-48 drop-shadow-2xl animate-fade-in-up">
          ...
          />
            ...
            {/* <h1 className="text-5xl md:text-7xl font-bold tracking-widest text-amber-500 uppercase drop-shadow-2xl mb-4 font-serif mt-2">
            Pankaj Studio
            </h1> */}
