    ...
    <>
              ...
              return (
                <div key={index} className="nav-item-dropdown" style={{ position: 'relative', display: 'inline-block' }}>
                  <span className="nav-link-premium" style={{ 
                    position: 'relative', fontFamily: 'var(--font-sans)', fontSize: 11, fontWeight: 500, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.85)', transition: 'all 0.3s ease', cursor: 'pointer', padding: '8px 16px' 
                  }}>
                    {l.label}
                  </span>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 40, background: '#050505', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 var(--page-gutter)', transform: menuOpen ? 'translateX(0)' : 'translateX(100%)', transition: 'transform 0.5s cubic-bezier(0.77,0,0.175,1)' }}>
        ...
        <Link href="/" onClick={() => setMenuOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 14, textDecoration: 'none', marginBottom: 50 }}>
          ...
          />
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <span style={{ fontFamily: '"Great Vibes", cursive', fontSize: 38, color: '#ffffff', lineHeight: 1, letterSpacing: '1px' }}>
              Pankaj
            </span>
            <span style={{ fontFamily: '"Montserrat", sans-serif', fontSize: 11, fontWeight: 400, color: '#d4af37', textTransform: 'uppercase', letterSpacing: '0.4em', marginTop: 4 }}>
              Studio
            </span>
          </div>
        </Link>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
          {MOBILE_LINKS.map((l) => (
            <Link key={l.href} href={l.href} onClick={() => setMenuOpen(false)} className="mobile-nav-link" style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(28px, 6vw, 40px)', fontWeight: 300, fontStyle: 'italic', color: 'rgba(255,255,255,0.7)', textDecoration: 'none', transition: 'all 0.3s ease' }}>
              {l.label}
            </Link>
          ))}
        </nav>
